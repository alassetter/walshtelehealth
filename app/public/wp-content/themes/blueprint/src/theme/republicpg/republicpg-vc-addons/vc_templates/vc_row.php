<?php
  ...
  if(!isset($GLOBALS['republicpg_vc_row_count'])) {
    $GLOBALS['republicpg_vc_row_count'] = 0;
  }
  $GLOBALS['republicpg_vc_row_count']++;

  
  //first row class
  $top_level_class = '';
  if(!is_single() && $GLOBALS['republicpg_vc_row_count'] == 1 && isset($post->ID)) {
      $republicpg_page_header_bool = republicpg_header_section_check($post->ID);
      if($republicpg_page_header_bool == false) {
	...
	if($mouse_based_parallax_bg == 'true'){
		wp_enqueue_script('republicpg_parallax');
	}
  ...
  $republicpg_using_VC_front_end_editor = ($republicpg_using_VC_front_end_editor == 'true') ? true : false;
	...
	if ( 'yes' !== $disable_element ) {
      ...
      if(!empty($zindex)) {
		...
		//equal height
		if($equal_height == 'yes' || $republicpg_using_VC_front_end_editor && strtolower($vertically_center_columns) == 'true')
    ...
    echo '</div>';
		...
		if($mouse_based_parallax_bg == 'true') {
	        ...
	        echo '<ul class="republicpg-parallax-scene" data-scene-position="'.$scene_position.'" data-scene-strength="'.$mouse_sensitivity.'">';
	        ...
	        //echo '<div class="republicpg-slider-loading '.$default_loader_class.'"> <span class="loading-icon '.$loading_animation.'"> '.$default_loader.'  </span> </div>';
		}
      ...
      //forced
			...
			<div class="republicpg-video-wrap" data-bg-alignment="'. esc_attr( $bg_position ) .'">';
				...
				if(!empty($video_external) && vc_extract_youtube_id( $video_external )) {
					...
					$video_markup .= '<div class="republicpg-youtube-bg"><span>'.$video_external.'</span></div>';
				} else {
					$video_markup .= '
					<video class="republicpg-video-bg" width="1800" height="700" '.$poster_markup.' preload="auto" loop autoplay '.$muted_video.'>';
      ...
      for($i=0; $i<$shape_divider_length;$i++) {
        ...
        $no_bg_color_class = (empty($shape_divider_color)) ? 'no-color ': '';
   			$shape_divider_markup .= '<div class="republicpg-shape-divider-wrap '.$no_bg_color_class.'" '. $shape_divider_height_val .' '.$using_percent_shape_divider_attr.' data-front="'. esc_attr( $shape_divider_bring_to_front ).'" data-style="'. esc_attr( $shape_type ).'" data-position="'. esc_attr( $shape_divider_pos[$i] ) .'" >';
        
  			switch($shape_type) {
  				case 'curve' : 
  					$shape_divider_markup .= '<svg class="republicpg-shape-divider" fill="'.$shape_divider_color.'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 100" preserveAspectRatio="none"> <path d="M 0 0 c 0 0 200 50 500 50 s 500 -50 500 -50 v 101 h -1000 v -100 z"></path> </svg>';
  					break;
  				case 'curve_asym' :
  					$shape_divider_markup .= '<svg class="republicpg-shape-divider" fill="'.$shape_divider_color.'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" preserveAspectRatio="none"> <path d="M0 100 C 20 0 50 0 100 100 Z"></path> </svg>';
  					break;
          case 'curve_asym_2' :
  					$shape_divider_markup .= '<svg class="republicpg-shape-divider" fill="'.$shape_divider_color.'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" preserveAspectRatio="none"> <path d="M0 100 C 50 0 80 0 100 100 Z"></path> </svg>';
  					break;
  				case 'tilt' :
  					$shape_divider_markup .= '<svg class="republicpg-shape-divider" fill="'.$shape_divider_color.'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 10" preserveAspectRatio="none"> <polygon points="104 10 0 0 0 10"></polygon> </svg>';
  					break;
          case 'tilt_alt' :
  					$shape_divider_markup .= '<svg class="republicpg-shape-divider" fill="'.$shape_divider_color.'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 10" preserveAspectRatio="none"> <polygon points="100 10 100 0 -4 10"></polygon> </svg>';
  					break;
  				case 'triangle' :
  					$shape_divider_markup .= '<svg class="republicpg-shape-divider" fill="'.$shape_divider_color.'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 100" preserveAspectRatio="none"> <polygon  points="501 53.27 0.5 0.56 0.5 100 1000.5 100 1000.5 0.66 501 53.27"/></svg>';
  					break;
          case 'fan' :
  					$shape_divider_markup .= '<svg class="republicpg-shape-divider" fill="'.$shape_divider_color.'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1003.92 91" preserveAspectRatio="none">  <polygon class="cls-1" points="502.46 46.31 1 85.67 1 91.89 1002.91 91.89 1002.91 85.78 502.46 46.31"/><polygon class="cls-2" points="502.46 45.8 1 0 1 91.38 1002.91 91.38 1002.91 0.1 502.46 45.8"/><rect class="cls-3" y="45.81" width="1003.92" height="46.09"/>
            </svg>';
  					break;
          case 'waves' :
  					...
  					break;
          case 'speech' :
  					$shape_divider_markup .= '<svg class="republicpg-shape-divider" fill="'.$shape_divider_color.'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 100" preserveAspectRatio="none"> <path d="M 0 45.86 h 458 c 29 0 42 19.27 42 19.27 s 13 -19.27 42.74 -19.27 h 457.26 v 54.14 h -1000 z"></path>  </svg>';
  					break;
          case 'straight_section' :
  					$shape_divider_markup .= '<svg class="republicpg-shape-divider" fill="'.$shape_divider_color.'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 10" preserveAspectRatio="none"> <polygon points="104 10, 104 0, 0 0, 0 10"> </svg>';
  					break;
          case 'clouds' :
          ...
          case 'waves_opacity_alt' :
  					$shape_divider_markup .= '<svg class="republicpg-shape-divider" fill="'.$shape_divider_color.'" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1000 300" preserveAspectRatio="none">  
