...
echo '<div class="republicpg-hor-list-item '.$hasbtn_class.'" data-font-family="'.$font_family.'" data-color="'.$hover_color.'" data-columns="'.$columns.'" data-column-layout="'.$column_layout_to_use.'">'; 
	for($i = 0; $i < intval($columns); $i++) {
		...
		if($index_to_grab == intval($columns)) {
			if(!empty($cta_1_text)) {
				...
				$cta_1_markup = '<a class="republicpg-list-item-btn" href="'.$cta_1_url.'" '.$btn_target_markup.'>'.$cta_1_text.'</a>';
			}
			if(!empty($cta_2_text)) {
				$btn_target_markup = (!empty($cta_2_open_new_tab) && $cta_2_open_new_tab == 'true' ) ? 'target="_blank"' : null;
				$cta_2_markup = '<a class="republicpg-list-item-btn second" href="'.$cta_2_url.'" '.$btn_target_markup.'>'.$cta_2_text.'</a>';
			}
		}

		echo '<div class="republicpg-list-item" data-text-align="'.$atts['col_'.$index_to_grab.'_text_align'].'">'.$atts['col_'.$index_to_grab.'_content'].$cta_1_markup.$cta_2_markup.'</div>';
	}
