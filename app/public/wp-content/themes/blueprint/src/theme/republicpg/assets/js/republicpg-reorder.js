jQuery(function($){
	...
	$('#sortable-table tbody').sortable({
		...
		update: function(event, ui) {
			...
			var nonce = $('#republicpg_meta_box_nonce').attr('value');
			var data = {
				action: 'republicpg_update_slide_order',
				postType: $(this).attr('data-post-type'),
				order: theOrder,
				republicpg_meta_box_nonce: nonce
			};
			$.post(ajaxurl,data);
		}
	}).disableSelection();
    ...
    if($('td.post-title').parent().hasClass('type-home_slider') || $('td.post-title').parent().hasClass('type-republicpg_slider')) {
