	...
	function wp_get_attachment( $attachment_id ) {
		...
		if(is_numeric($attachment_id) && $attachment_id > 0) {
			...
			return array(
				...
				'masonry_image_sizing' => get_post_meta( $attachment->ID, 'republicpg_image_gal_masonry_sizing', true ),
				'image_url' => get_post_meta( $attachment->ID, 'republicpg_image_gal_url', true ),
			);
		} else {
			return array(
				'alt' => '',
...
if ( $type == 'flexslider_style' ) {
...
} else if ( $type == 'republicpgslider_style' ) {
  ...
  if($republicpg_using_VC_front_end_editor) {
	...
	$slides_wrap_start .= '<div class="republicpg-slider-wrap" style="height: '.$slide_height.'px" data-flexible-height="'.$flexible_slider_height.'" data-fullscreen="false"  data-full-width="false" data-parallax="false" '.$autorotation_attr.' id="ns-id-'.uniqid().'">';
	...
	if($hide_arrow_navigation != true) {
		$slides_wrap_end .= '<a href="" class="slider-prev"><i class="icon-blueprint-left-arrow"></i> <div class="slide-count"> <span class="slide-current">1</span> <i class="icon-blueprint-right-line"></i> <span class="slide-total"></span> </div> </a>
	...
	$slides_wrap_end .= '<div class="republicpg-slider-loading"></div>';
...
foreach ( $images as $attach_id ) {
	...
	if($type == 'republicpgslider_style') {
								...
								<div class="work-info"> 
									...
									<?php 
										if($attach_id > 0) { 
											...
											if(!empty($attachment_meta['image_url'])) {
												echo '<div class="vert-center no-text"><a class="no-text" href="'.$attachment_meta['image_url'].'">'.__("View Larger", 'blueprint').'</a> ';
											} else {
												echo '<div class="vert-center"><a ';
												 if(!empty($attachment_meta['description'])) echo 'title="'.$attachment_meta['description'].'"';
												echo ' href="'.$p_img_large[0].'" class="default-link pretty_photo">'.__("View Larger", 'blueprint').'</a> ';
											} ?>
											</div><!--/vert-center-->
										<?php } ?>
