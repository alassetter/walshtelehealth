			...
			if ( ! empty( $excluded_categories ) ) {
				if ( ! is_array( $excluded_categories ) ) {
					...
					if ( strpos( $excluded_categories, ' and ' ) !== false ) {
						_deprecated_argument( __FUNCTION__, '3.3', sprintf( __( 'Use commas instead of %s to separate excluded categories.','blueprint' ), "'and'" ) );
...
if( !function_exists('be_adjacent_post_link') ) {
	...
	function be_adjacent_post_link($format, $link, $in_same_cat = false, $excluded_categories = '', $previous = true, $taxonomy = 'category') {
		...
		if ( empty($post->post_title) )
			$title = $previous ? esc_html__('Previous Post','blueprint') : esc_html__('Next Post','blueprint');
...
if ( !function_exists( 'republicpg_pagination' ) ) {
	
	function republicpg_pagination() {
		
		global $republicpg_options;
		//extra pagination
		if( !empty($republicpg_options['extra_pagination']) && $republicpg_options['extra_pagination'] == '1' ){
			    ...
			    if ($total_pages > 1){  
				  ...
				  echo '<div id="pagination" data-is-text="'.esc_attr__("All items loaded", 'blueprint').'">';
			      ...
			      echo paginate_links(array(  
			          ...
			          'current' => $current,  
			          'total' => $total_pages,  
			          'prev_text'    => esc_html__('Previous','blueprint'),
    				  	'next_text'    => esc_html__('Next','blueprint')
			        )); 
