...
(function($) {
    ...
    $.redux.wbc_importer = function() {
        ...
        //demo depends
        var republicpg_demo_depends = {
          "Agency" : {
            "plugins": ['js_composer_blueprint']
          },
          "App" : {
            "plugins": ['js_composer_blueprint']
          },
          "Ascend" : {
            "plugins": ['js_composer_blueprint','woocommerce']
          },
          "Band" : {
            "plugins": ['js_composer_blueprint']
          },
          ...
          "Ecommerce-Ultimate" : {
            "plugins": ['js_composer_blueprint','woocommerce','yith-woocommerce-ajax-navigation']
          },
          "Ecommerce-Creative" : {
            "plugins": ['js_composer_blueprint','woocommerce','yith-woocommerce-ajax-navigation', 'popup-maker']
          },
        ...
        $('.wrap-importer.theme.not-imported, #wbc-importer-reimport').unbind('click').on('click', function(e) {
          ...
          $('.republicpg-demo-importer-selection-modal .cnkt-plugin-installer .plugin, .republicpg-demo-importer-selection-modal .cnkt-plugin-installer').hide();
          
          $.each(republicpg_demo_depends,function(k,v){
            ...
            if(k == $selected_demo_block_name) {
              ...
              if(typeof plugins_required != 'undefined' && plugins_required.length > 0) {
                ...
                for(var i=0; i<plugins_required.length; i++) {
                  ...
                  $('.republicpg-demo-importer-selection-modal .cnkt-plugin-installer h4[data-slug]').each(function(){
          ...
          //show modal
          $('.republicpg-demo-importer-selection-modal .switch-options.blueprint').addClass('activated');
          $('.republicpg-demo-importer-selection-modal-backdrop, .republicpg-demo-importer-selection-modal').fadeIn(200);
          
          
          //set preview img
          $('.republicpg-demo-importer-selection-modal .republicpg-preview-img').css('background-image','url('+ $selected_demo_block.find('.wbc_image').attr('src') +')');
          $('.republicpg-demo-importer-selection-modal .republicpg-demo-preview-header h2').text($selected_demo_block_name);
          
        });
        
        //close popup
        $('.republicpg-demo-importer-selection-modal a.close').unbind('click').on('click', function(e) {
          
          e.preventDefault();
          $('.republicpg-demo-importer-selection-modal-backdrop, .republicpg-demo-importer-selection-modal').fadeOut(250);
          
        });
        
        
        //selected classes
        $('.republicpg-demo-importer-selection-modal a.theme-demo-import-option').unbind('click').on('click', function(e) {
          e.preventDefault();
          $(this).parents('.demo-importer-form-row').find('.switch-options.blueprint').toggleClass('activated');
        });
        
        $('.republicpg-demo-importer-selection-modal .switch-options.blueprint').unbind('click').on('click', function(e) {
          e.preventDefault();
          $(this).toggleClass('activated');
        });
        
        //import demo
        $('.republicpg-demo-importer-selection-modal a.submit').unbind('click').on('click', function(e) {
          
            e.preventDefault();
            
            if($selected_demo.length == 0) { return; }
            
            var $modal = $(this).parents('.republicpg-demo-importer-selection-modal');
            ...
            //close modal
            $('.republicpg-demo-importer-selection-modal a.close').trigger('click');
            ...
            data.import_demo_content = ($modal.find('.import-republicpg-theme-demo-content > .switch-options.activated').length > 0) ? 'true' : 'false';
            data.import_theme_option_settings = ($modal.find('.import-republicpg-theme-option-settings > .switch-options.activated').length > 0) ? 'true' : 'false';
            data.import_demo_widgets = ($modal.find('.import-republicpg-theme-demo-widgets > .switch-options.activated').length > 0) ? 'true' : 'false';
