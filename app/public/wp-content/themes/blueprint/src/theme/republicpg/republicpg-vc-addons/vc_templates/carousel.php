...
if( isset($_GET['vc_editable']) ) {
	...
	if($republicpg_using_VC_front_end_editor && $script != 'flickity') {
		$script = 'flickity';
	}
}

$GLOBALS['republicpg-carousel-script'] = $script;
$GLOBALS['republicpg_carousel_column_color'] = $column_color;
...
if($script == 'carouFredSel') {
	...
	if($flickity_formatting == 'fixed_text_content_fullwidth') {
		echo '<div class="republicpg-carousel-flickity-fixed-content"> <div class="republicpg-carousel-fixed-content">';
		...
		if(!empty($cta_button_text)) {
			...
			echo '<div><a class="republicpg-button large regular '. $button_color .  $regular_btn_class . ' has-icon" href="'.$cta_button_url.'" '.$btn_target_markup.' data-color-override="false" data-hover-color-override="false" data-hover-text-color-override="#fff">'.$btn_text_markup.'</a></div>';
		}
