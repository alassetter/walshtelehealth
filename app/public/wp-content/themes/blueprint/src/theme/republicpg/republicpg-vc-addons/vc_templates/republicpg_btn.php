...
$target = ($open_new_tab == 'true') ? 'target="_blank"' : null;
	...
	$headerFormat = (!empty($republicpg_options['header_format'])) ? $republicpg_options['header_format'] : 'default';
	...
	switch ($size) {
		...
		case 'small' :
			$button_open_tag .= '<a class="republicpg-button small '. $style_color . $has_icon . $stnd_button.'" style="'. $margins . $starting_custom_color.'" '. $target;
			break;
		case 'medium' :
			$button_open_tag .= '<a class="republicpg-button medium ' . $style_color . $has_icon . $stnd_button.'" style="'. $margins . $starting_custom_color.'" '. $target;
			break;
		case 'large' :
			$button_open_tag .= '<a class="republicpg-button large '. $style_color . $has_icon . $stnd_button.'" style="'.$margins . $starting_custom_color.'" '. $target;
			break;	
		case 'jumbo' :
			$button_open_tag .= '<a class="republicpg-button jumbo '. $style_color . $has_icon . $stnd_button.'" style="' . $margins . $starting_custom_color.'" '. $target;
			break;	
		case 'extra_jumbo' :
			$button_open_tag .= '<a class="republicpg-button extra_jumbo '. $style_color . $has_icon . $stnd_button.'" style="' . $margins . $starting_custom_color.'" '. $target;
			break;	
	}
	...
	if($button_style != 'see-through-3d') {
		...
		echo '
		<div class="republicpg-3d-transparent-button" style="'.$margins.'" data-size="'.$size.'">
