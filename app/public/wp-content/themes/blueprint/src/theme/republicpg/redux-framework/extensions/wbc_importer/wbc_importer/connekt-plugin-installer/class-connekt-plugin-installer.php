...
if( !class_exists('Connekt_Plugin_Installer') ) {
   ...
   class Connekt_Plugin_Installer {
      ...
      */
      public static function init($plugins){ ?>
         ...
         <?php
            ...
            echo '<h3>'. esc_html__('Required Plugins','blueprint').'</h3>';
            ...
            require_once( ABSPATH . 'wp-admin/includes/plugin-install.php' );
   		   ...
   		   foreach($plugins as $plugin) :
               ...
               $button_text = __('Install Now', 'blueprint');
               ...
               //echo '</pre>';
                  ...
                  /*republicpg addition */ 
	   	            if(self::republicpg_check_plugin_active($plugin['slug'])){
                  /*republicpg addition end */ 
	      	            // plugin activation, confirmed!
	                  	$button_classes = 'button disabled';
	                  	$button_text = __('Activated', 'blueprint');
	                  } else {
	                     // It's installed, let's activate it
	                  	$button_classes = 'activate button button-primary';
	                  	$button_text = __('Activate', 'blueprint');
	                  }
	               }
    ...
    /*republicpg addition*/
    public static function republicpg_check_plugin_active($plugin_slug) {
      
      switch($plugin_slug) {
        
         case 'js_composer_blueprint':
    ...
    /*republicpg addition end*/
      ...
      */
		public function cnkt_plugin_installer(){
			...
			if ( ! current_user_can('install_plugins') )
				wp_die( __( 'Sorry, you are not allowed to install plugins on this site.', 'blueprint' ) );
      ...
      $plugin_name = $_POST["plugin_title"];
			...
			if (! wp_verify_nonce( $nonce, 'cnkt_installer_nonce' ))
				wp_die( __( 'Error - unable to verify nonce, please try again.', 'blueprint') );
      ...
      */
		public function cnkt_plugin_activation(){
			if ( ! current_user_can('install_plugins') )
				wp_die( __( 'Sorry, you are not allowed to activate plugins on this site.', 'blueprint' ) );
      ...
      $plugin_name = $_POST["plugin_title"];
			...
			// Check our nonce, if they don't match then bounce!
			if (! wp_verify_nonce( $nonce, 'cnkt_installer_nonce' ))
				die( __( 'Error - unable to verify nonce, please try again.', 'blueprint' ) );
      ...
      */
      ...
      wp_enqueue_script( 'plugin-installer', CNKT_INSTALLER_PATH. 'assets/installer.js', array( 'jquery' ));
			wp_localize_script( 'plugin-installer', 'cnkt_installer_localize', array(
               ...
               'install_now' => __('Are you sure you want to install this plugin?', 'blueprint'),
               'install_btn' => __('Install Now', 'blueprint'),
               'activate_btn' => __('Activate', 'blueprint'),
               'installed_btn' => __('Activated', 'blueprint')
            ));
