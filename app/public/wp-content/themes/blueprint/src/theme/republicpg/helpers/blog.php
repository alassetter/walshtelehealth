<?php
/**
 * Blueprint blog related functions
 *
...
if ( ! function_exists( 'republicpg_excerpt_more' ) ) {
	function republicpg_excerpt_more( $more ) {
		return '...';
	}
}
add_filter( 'excerpt_more', 'republicpg_excerpt_more' );
...
if ( ! function_exists( 'republicpg_set_post_views' ) ) {

	function republicpg_set_post_views() {
		...
		if ( get_post_type() == 'post' && is_single() ) {
			...
			if ( ! empty( $post_id ) ) {
				...
				$the_view_count = get_post_meta( $post_id, 'republicpg_blog_post_view_count', true );

				if ( $the_view_count != '' ) {

					$the_view_count = intval( $the_view_count );
					$the_view_count++;
					update_post_meta( $post_id, 'republicpg_blog_post_view_count', $the_view_count );

				} else {

					$the_view_count = 0;
					delete_post_meta( $post_id, 'republicpg_blog_post_view_count' );
					add_post_meta( $post_id, 'republicpg_blog_post_view_count', '0' );
...
add_action( 'wp_head', 'republicpg_set_post_views' );
...
if ( ! function_exists( 'republicpg_excerpt' ) ) {

	function republicpg_excerpt( $limit ) {
...
function republicpg_remove_more_jump_link( $link ) {
...
add_filter( 'the_content_more_link', 'republicpg_remove_more_jump_link' );
...
function republicpg_remove_category_list_rel( $output ) {
	// Remove rel attribute from the category list
	return str_replace( ' rel="category tag"', '', $output );
}

add_filter( 'wp_list_categories', 'republicpg_remove_category_list_rel' );
add_filter( 'the_category', 'republicpg_remove_category_list_rel' );
...
function republicpg_blog_social_sharing() {
	...
	$blog_header_type                = ( ! empty( $republicpg_options['blog_header_type'] ) ) ? $republicpg_options['blog_header_type'] : 'default';
	...
	if ( ! empty( $republicpg_options['blog_social'] ) && $republicpg_options['blog_social'] == 1 || $fullscreen_header == true ) {

		  echo '<div class="republicpg-social ' . $fullscreen_class . '">';

		if ( $fullscreen_header == false ) {

			if ( $remove_single_post_republicpg_love != '1' ) {
				echo '<span class="n-shortcode">' . republicpg_love( 'return' ) . '</span>';
			}

			// facebook
			if ( ! empty( $republicpg_options['blog-facebook-sharing'] ) && $republicpg_options['blog-facebook-sharing'] == 1 ) {
				echo "<a class='facebook-share republicpg-sharing' href='#' title='" . esc_attr__( 'Share this', 'blueprint' ) . "'>  <i class='fa fa-facebook'></i> <span class='social-text'>" . esc_html__( 'Share', 'blueprint' ) . "</span> <span class='count'></span></a>";
			}
			// twitter
			if ( ! empty( $republicpg_options['blog-twitter-sharing'] ) && $republicpg_options['blog-twitter-sharing'] == 1 ) {
				echo "<a class='twitter-share republicpg-sharing' href='#' title='" . esc_attr__( 'Tweet this', 'blueprint' ) . "'> <i class='fa fa-twitter'></i> <span class='social-text'>" . esc_html__( 'Tweet', 'blueprint' ) . "</span> <span class='count'></span></a>";
			}

			// google plus
			if ( ! empty( $republicpg_options['blog-google-plus-sharing'] ) && $republicpg_options['blog-google-plus-sharing'] == 1 ) {
				echo "<a class='google-plus-share republicpg-sharing-alt' href='#' title='" . esc_attr__( 'Share this', 'blueprint' ) . "'> <i class='fa fa-google-plus'></i> <span class='social-text'>" . esc_html__( 'Share', 'blueprint' ) . "</span> <span class='count'>0</span></a>";
			}

			// linkedIn
			if ( ! empty( $republicpg_options['blog-linkedin-sharing'] ) && $republicpg_options['blog-linkedin-sharing'] == 1 ) {
				echo "<a class='linkedin-share republicpg-sharing' href='#' title='" . esc_attr__( 'Share this', 'blueprint' ) . "'> <i class='fa fa-linkedin'></i> <span class='social-text'>" . esc_html__( 'Share', 'blueprint' ) . "</span> <span class='count'> </span></a>";
			}

			// pinterest
			if ( ! empty( $republicpg_options['blog-pinterest-sharing'] ) && $republicpg_options['blog-pinterest-sharing'] == 1 ) {
				echo "<a class='pinterest-share republicpg-sharing' href='#' title='" . esc_attr__( 'Pin this', 'blueprint' ) . "'> <i class='fa fa-pinterest'></i> <span class='social-text'>" . esc_html__( 'Pin', 'blueprint' ) . "</span> <span class='count'></span></a>";
			}
		} else {
			// facebook
...
function republicpg_next_post_display() {

	 global $post;
	 global $republicpg_options;

	 $post_header_style            = ( ! empty( $republicpg_options['blog_header_type'] ) ) ? $republicpg_options['blog_header_type'] : 'default';
	 $post_pagination_style        = ( ! empty( $republicpg_options['blog_next_post_link_style'] ) ) ? $republicpg_options['blog_next_post_link_style'] : 'fullwidth_next_only';
	...
	if ( ! empty( $next_post ) && ! empty( $republicpg_options['blog_next_post_link'] ) && $republicpg_options['blog_next_post_link'] == '1' ||
	   $post_pagination_style == 'contained_next_prev' && ! empty( $republicpg_options['blog_next_post_link'] ) && $republicpg_options['blog_next_post_link'] == '1' ||
		   $post_pagination_style == 'fullwidth_next_prev' && ! empty( $republicpg_options['blog_next_post_link'] ) && $republicpg_options['blog_next_post_link'] == '1' ) { ?>
			...
			<div data-post-header-style="<?php echo esc_attr( $post_header_style ); ?>" class="blog_next_prev_buttons wpb_row vc_row-fluid <?php echo esc_attr( $full_width_content_class ); ?> standard_section" data-style="<?php echo esc_attr( $post_pagination_style_output ); ?>" data-midnight="light">
				...
				if ( ! empty( $next_post ) ) {
					  $bg       = get_post_meta( $next_post->ID, '_republicpg_header_bg', true );
					  $bg_color = get_post_meta( $next_post->ID, '_republicpg_header_bg_color', true );
				} else {
					$bg       = '';
					$bg_color = '';
				}
				...
				if ( $post_pagination_style == 'fullwidth_next_prev' || $post_pagination_style == 'contained_next_prev' ) {
					...
					if ( ! empty( $previous_post ) ) {
						...
						if ( ! empty( $bg ) ) {
							...
							echo '<a href="' . esc_url( get_permalink( $previous_post_id ) ) . '"></a><h3><span>' . esc_html__( 'Previous Post', 'blueprint' ) . '</span><span class="text">' . wp_kses_post( $previous_post->post_title ) . '
					...
					if ( ! empty( $next_post ) ) {
						...
						if ( ! empty( $bg ) ) {
							...
							echo '<a href="' . esc_url( get_permalink( $next_post_id ) ) . '"></a><h3><span>' . esc_html__( 'Next Post', 'blueprint' ) . '</span><span class="text">' . wp_kses_post( $next_post->post_title ) . '
					<svg class="next-arrow" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 39 12"><line class="top" x1="23" y1="-0.5" x2="29.5" y2="6.5" stroke="#ffffff;"></line><line class="bottom" x1="23" y1="12.5" x2="29.5" y2="5.5" stroke="#ffffff;"></line></svg><span class="line"></span></span></h3>';
					...
					?>
						...
						<div class="inner">
							<span><?php echo '<i>' . esc_html__( 'Next Post', 'blueprint' ) . '</i>'; ?></span>
...
function republicpg_related_post_display() {

	global $post;
	global $republicpg_options;

	$using_related_posts = ( ! empty( $republicpg_options['blog_related_posts'] ) && ! empty( $republicpg_options['blog_related_posts'] ) == '1' ) ? true : false;
	...
	if ( $current_categories ) {
		...
		$related_title_text        = esc_html__( 'Related Posts', 'blueprint' );
		$related_post_title_option = ( ! empty( $republicpg_options['blog_related_posts_title_text'] ) ) ? wp_kses_post( $republicpg_options['blog_related_posts_title_text'] ) : 'Related Posts';
		...
		switch ( $related_post_title_option ) {
			...
			case 'similar_posts':
				$related_title_text = esc_html__( 'Similar Posts', 'blueprint' );
				break;

			case 'you_may_also_like':
				$related_title_text = esc_html__( 'You May Also Like', 'blueprint' );
				break;
			case 'recommended_for_you':
				$related_title_text = esc_html__( 'Recommended For You', 'blueprint' );
				break;
		}
		...
		$using_post_pag     = ( ! empty( $republicpg_options['blog_next_post_link'] ) && $republicpg_options['blog_next_post_link'] == '1' ) ? 'true' : 'false';
		$related_post_style = ( ! empty( $republicpg_options['blog_related_posts_style'] ) ) ? esc_html( $republicpg_options['blog_related_posts_style'] ) : 'material';
...
if ( ! function_exists( 'republicpg_remove_hentry_cssclass' ) ) {
	function republicpg_remove_hentry_cssclass( $classes ) {
		$classes = array_diff( $classes, array( 'hentry' ) );
		return $classes;
	}
}
add_filter( 'post_class', 'republicpg_remove_hentry_cssclass' );
