...
if(!function_exists('republicpg_set_vc_as_theme')) {
	function republicpg_set_vc_as_theme() {

		vc_set_as_theme($disable_updater = true);
		$template_directory = get_template_directory();

		if(defined( 'BLUEPRINT_VC_ACTIVE')) {
		    $child_dir = $template_directory . '/republicpg/republicpg-vc-addons/vc_templates';
		    $parent_dir = $template_directory . '/republicpg/republicpg-vc-addons/vc_templates';
		...
		//only allow frontend editor when using Blueprint WPBakery that supports it
...
add_action('vc_before_init', 'republicpg_set_vc_as_theme');




add_filter( 'vc_load_default_templates', 'republicpg_custom_template_modify_array' ); // Hook in
function republicpg_custom_template_modify_array( $data ) {
    return array();
}
...
//remove WC elements
function blueprint_vc_remove_woocommerce() {
...
add_action( 'vc_build_admin_page', 'blueprint_vc_remove_woocommerce', 11 );




if ( !function_exists( 'republicpg_vc_navbar_mod' ) ) {

	function republicpg_vc_navbar_mod($list) {
			...
			if ( is_array( $list ) ) {
				...
				// $list[] = array( 'blueprint_studio', republicpg_generate_blueprint_studio_button() );
				$list[] = array( 'user_templates', republicpg_generate_user_template_button() );
			}
...
if ( !function_exists( 'republicpg_generate_blueprint_studio_button' ) ) {
	function republicpg_generate_blueprint_studio_button() {
		return '<li><a href="javascript:;" class="vc_icon-btn vc_templates-button blueprint-studio-templates"  id="vc_templates-editor-button" title="'
					 . esc_html__('Blueprint studio template library', 'blueprint' ) . '"><i class="vc-composer-icon vc-c-icon-add_template"></i> <span>'. esc_html__('Blueprint Templates','blueprint'). '</span></a></li>';
	}
}

if ( !function_exists( 'republicpg_generate_user_template_button' ) ) {
	function republicpg_generate_user_template_button() {
		return '<li><a href="javascript:;" class="vc_icon-btn vc_templates-button user-templates"  id="vc_templates-editor-button" title="'
					 . esc_html__('My templates', 'blueprint' ) . '"><i class="vc-composer-icon vc-c-icon-add_template"></i> <span>'. esc_html__('My Templates','blueprint'). '</span></a></li>';
	}
}

add_filter('vc_nav_controls','republicpg_vc_navbar_mod');
add_filter('vc_nav_front_controls','republicpg_vc_navbar_mod');
...
if(function_exists('vc_add_shortcode_param')) {
	...
	vc_add_shortcode_param( 'dropdown_multi', 'dropdown_multi_settings_field', get_template_directory_uri().'/republicpg/republicpg-vc-addons/js/backend-edit-form-bulk.js' );
	...
	vc_add_shortcode_param( 'republicpg_radio_image', 'republicpg_radio_images_settings_field' );
	function republicpg_radio_images_settings_field( $param, $value ) {
			$rnd_id = uniqid();
			$options = isset($param['options']) ? $param['options'] : '';

			$param_line = '';
			$param_line .= '<input type="hidden" id="republicpg-radio-image-'.$rnd_id.'" class="wpb_vc_param_value '.esc_attr($param['param_name']).' '.esc_attr($param['type']).'" name="'.esc_attr($param['param_name']).'" value="'.esc_attr($value).'"/>';
	        $param_line .= '<div class="republicpg-radio-image" data-grp-id="' . $rnd_id . '">';
	        $param_line .= '<ul class="republicpg_radio_images_list">';
	...
	function hotspot_image_preview_field( $settings, $value ) {
	   ...
	   return '<div id="republicpg_image_with_hotspots_preview"><input name="' . esc_attr( $settings['param_name'] ) . '" type="hidden" class="wpb_vc_param_value ' . esc_attr( $settings['param_name'] ) . '" value="'.$value.'" /> '.$image_output. '</div>';
	}
}

// VC_Row

add_action('vc_before_init', 'republicpg_custom_maps');

function republicpg_custom_maps() {

	$vc_is_wp_version_3_6_more = version_compare(preg_replace('/^([\d\.]+)(\-.*$)/', '$1', get_bloginfo('version')), '3.6') >= 0;
	global $republicpg_options;
	$is_admin = is_admin();
	$republicpg_template_dir = get_template_directory();
	...
	vc_map( array(
		...
		'params' => array(
			...
			array(
				...
				"description" => "Can be used as an alternative to self hosted videos. Enter full video URL e.g. https://www.youtube.com/watch?v=qw1zHvoWxtc",
				"dependency" => Array('element' => "video_bg", 'value' => array('use_video'))
			),
			...
			array(
				...
				"group" => "Shape Divider",
				"options" => array(
					"curve" => array( esc_html__('Curve', 'blueprint') => $republicpg_get_template_directory_uri."/republicpg/republicpg-vc-addons/img/shape_dividers/curve_down.jpg"),
					'fan' => array( esc_html__('Fan', 'blueprint') => $republicpg_get_template_directory_uri."/republicpg/republicpg-vc-addons/img/shape_dividers/fan.jpg"),
					'curve_opacity' => array( esc_html__('Curve Opacity', 'blueprint') => $republicpg_get_template_directory_uri."/republicpg/republicpg-vc-addons/img/shape_dividers/curve_opacity.jpg"),
					"mountains" => array( esc_html__('Mountains', 'blueprint') => $republicpg_get_template_directory_uri."/republicpg/republicpg-vc-addons/img/shape_dividers/mountains.jpg"),
					'curve_asym' => array( esc_html__('Curve Asym.', 'blueprint') => $republicpg_get_template_directory_uri."/republicpg/republicpg-vc-addons/img/shape_dividers/curve_asym.jpg"),
					'curve_asym_2' => array( esc_html__('Curve Asym. Alt', 'blueprint') => $republicpg_get_template_directory_uri."/republicpg/republicpg-vc-addons/img/shape_dividers/curve_asym_2.jpg"),
					"tilt" => array( esc_html__('Tilt', 'blueprint') => $republicpg_get_template_directory_uri."/republicpg/republicpg-vc-addons/img/shape_dividers/tilt.jpg"),
					"tilt_alt" => array( esc_html__('Tilt Alt', 'blueprint') => $republicpg_get_template_directory_uri."/republicpg/republicpg-vc-addons/img/shape_dividers/tilt_alt.jpg"),
					"triangle" => array( esc_html__('Triangle', 'blueprint') => $republicpg_get_template_directory_uri."/republicpg/republicpg-vc-addons/img/shape_dividers/triangle.jpg"),
					'waves' => array( esc_html__('Waves', 'blueprint') => $republicpg_get_template_directory_uri."/republicpg/republicpg-vc-addons/img/shape_dividers/waves_no_opacity.jpg"),
					'waves_opacity' => array( esc_html__('Waves Opacity', 'blueprint') => $republicpg_get_template_directory_uri."/republicpg/republicpg-vc-addons/img/shape_dividers/waves.jpg"),
					'waves_opacity_alt' => array( esc_html__('Waves Opacity 2', 'blueprint') => $republicpg_get_template_directory_uri."/republicpg/republicpg-vc-addons/img/shape_dividers/waves_opacity.jpg"),
					'clouds' => array( esc_html__('Clouds', 'blueprint') => $republicpg_get_template_directory_uri."/republicpg/republicpg-vc-addons/img/shape_dividers/clouds.jpg"),
					"speech" => array( esc_html__('Speech', 'blueprint') => $republicpg_get_template_directory_uri."/republicpg/republicpg-vc-addons/img/shape_dividers/speech.jpg"),
					"straight_section" => array( esc_html__('Straight Section', 'blueprint') => $republicpg_get_template_directory_uri."/republicpg/republicpg-vc-addons/img/shape_dividers/straight_section.jpg")
				),
			),
			array(
				"type" => "colorpicker",
	...
	if(!empty($republicpg_options['header-inherit-row-color']) && $republicpg_options['header-inherit-row-color'] == '1') {
  ...
  //sidebar
	...
	vc_lean_map('full_width_section', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/full_width_section.php');
	...
	if($republicpg_disable_republicpg_slider != true) {
		vc_lean_map('republicpg_slider', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_slider.php');
	}

	// Horizontal progress bar shortcode
	vc_lean_map('bar', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/bar.php');


	// Split Line Heading
	class WPBakeryShortCode_Split_Line_Heading extends WPBakeryShortCode { }

	vc_lean_map('split_line_heading', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/split_line_heading.php');



	// Split Line Heading
	class WPBakeryShortCode_Republicpg_Highlighted_Text extends WPBakeryShortCode { }

	vc_lean_map('republicpg_highlighted_text', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_highlighted_text.php');



	// Divider
	vc_lean_map('divider', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/divider.php');


	// Single image
	vc_lean_map('image_with_animation', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/image_with_animation.php');


	//cascading images
	class WPBakeryShortCode_Republicpg_Cascading_Images extends WPBakeryShortCode {}

	vc_lean_map('republicpg_cascading_images', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_cascading_images.php');



	// Image Comparision
	class WPBakeryShortCode_Republicpg_Image_Comparison extends WPBakeryShortCode {}

	vc_lean_map('republicpg_image_comparison', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_image_comparison.php');


	// Portfolio
	vc_lean_map('republicpg_portfolio', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_portfolio.php');

	vc_lean_map('recent_projects', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/recent_projects.php');


	//Horizontal List Item
	class WPBakeryShortCode_Republicpg_Horizontal_List_Item extends WPBakeryShortCode {}
	vc_lean_map('republicpg_horizontal_list_item', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_horizontal_list_item.php');


	// Blog
	vc_lean_map('republicpg_blog', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_blog.php');

	vc_lean_map('recent_posts', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/recent_posts.php');
	...
	if($woocommerce) {
		...
		class WPBakeryShortCode_Republicpg_Woo_Products extends WPBakeryShortCode {

		}

		vc_lean_map('republicpg_woo_products', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_woo_products.php');
	...
	//category grid
	class WPBakeryShortCode_Republicpg_Category_Grid extends WPBakeryShortCode {

	}

	/*helper function*/
	if(!function_exists('republicpg_grid_item_markup')) {

		function republicpg_grid_item_markup($temp_cat_obj_holder,$atts) {
		    ...
		    if($temp_cat_obj_holder) {
		        ...
		        $markup .= '<div class="republicpg-category-grid-item"> <div class="inner"> <a class="republicpg-category-grid-link" href="'. get_term_link($temp_cat_obj_holder->term_id) .'"></a>';
		        $markup .= '<div class="republicpg-category-grid-item-bg" '.$bg_style_markup.'></div>';
		        ...
		        if($atts['subtext'] == 'cat_item_count') {
		          ...
		          if($atts['post_type'] == 'posts') {
		            if($temp_cat_obj_holder->count == 1) { $subtext_count_markup = '<span class="subtext">' . $temp_cat_obj_holder->count .  ' ' . esc_html__('post', 'blueprint') . '</span>'; }
		            else { $subtext_count_markup = '<span class="subtext">' . $temp_cat_obj_holder->count .  ' ' . esc_html__('posts', 'blueprint') . '</span>'; }
		          } else if($atts['post_type'] == 'products') {
		            if($temp_cat_obj_holder->count == 1) {  $subtext_count_markup = '<span class="subtext">' . $temp_cat_obj_holder->count .  ' ' . esc_html__('product', 'blueprint') . '</span>'; }
		            else { $subtext_count_markup = '<span class="subtext">' . $temp_cat_obj_holder->count .  ' ' . esc_html__('products', 'blueprint') . '</span>';  }
		          }
	...
	vc_lean_map('republicpg_category_grid', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_category_grid.php');



	// Centered Heading
	vc_lean_map('heading', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/heading.php');

	// Call to action
	class WPBakeryShortCode_Republicpg_Cta extends WPBakeryShortCode {}
	vc_lean_map('republicpg_cta', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_cta.php');

	// video lightbox
	class WPBakeryShortCode_Republicpg_Video_Lightbox extends WPBakeryShortCode {}
	vc_lean_map('republicpg_video_lightbox', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_video_lightbox.php');


	// Milestone
	vc_lean_map('milestone', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/milestone.php');

	// Google Map
	class WPBakeryShortCode_Republicpg_Gmap extends WPBakeryShortCode {
	}
	vc_lean_map('republicpg_gmap', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_gmap.php');

	// Team Member
	vc_lean_map('team_member', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/team_member.php');

	// Home Builder
	vc_lean_map('home_builder', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/home_builder.php');

	// Fancy Box
	class WPBakeryShortCode_Fancy_Box extends WPBakeryShortCode { }
	vc_lean_map('fancy_box', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/fancy_box.php');

	// Flip Box
	class WPBakeryShortCode_Republicpg_Flip_Box extends WPBakeryShortCode { }
	vc_lean_map('republicpg_flip_box', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/flip-box.php');


	// Gradient Text
	class WPBakeryShortCode_Republicpg_Gradient_Text extends WPBakeryShortCode { }
	vc_lean_map('republicpg_gradient_text', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/gradient-text.php');


	// Hotspot
	class WPBakeryShortCode_Republicpg_Image_With_Hotspots extends WPBakeryShortCode { }
	vc_lean_map('republicpg_image_with_hotspots', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_image_with_hotspots.php');


	class WPBakeryShortCode_Republicpg_Hotspot extends WPBakeryShortCode { }
	vc_lean_map('republicpg_hotspot', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_hotspot.php');


	// Fancy Title
	class WPBakeryShortCode_Republicpg_Animated_Title extends WPBakeryShortCode { }
	vc_lean_map('republicpg_animated_title', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/fancy-title.php');


	// Single Testimonial
	class WPBakeryShortCode_Republicpg_Single_Testimonial extends WPBakeryShortCode { }
	vc_lean_map('republicpg_single_testimonial', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_single_testimonial.php');
	...
	/* Accordion block
	---------------------------------------------------------- */
	vc_lean_map('toggles', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/toggles.php');
	vc_lean_map('toggle', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/toggle.php');
	...
	/* Tabs
	---------------------------------------------------------- */
	vc_lean_map('tabbed_section', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/tabbed_section.php');
	vc_lean_map('tab', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/tab.php');



	class WPBakeryShortCode_Testimonial_Slider extends WPBakeryShortCode_Tabbed_Section { }

	vc_lean_map('testimonial_slider', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/testimonial_slider.php');
	...
	vc_lean_map('testimonial', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/testimonial.php');
    ...
    vc_lean_map( 'vc_wp_custommenu', null,  $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/shortcode-vc-wp-custommenu.php' );


	/* clients slider */
	class WPBakeryShortCode_Clients extends WPBakeryShortCode_Tabbed_Section { }

	vc_lean_map('clients', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/clients.php');
	...
	vc_lean_map('client', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/client.php');




	/* icon list */
	class WPBakeryShortCode_Republicpg_Icon_List extends WPBakeryShortCode_Tabbed_Section { }

	vc_lean_map('republicpg_icon_list', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_icon_list.php');



	class WPBakeryShortCode_Republicpg_Icon_List_Item extends WPBakeryShortCode {
	...
	vc_lean_map('republicpg_icon_list_item', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_icon_list_item.php');
	...
	vc_lean_map('page_submenu', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/page_submenu.php');
	...
	vc_lean_map('page_link', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/page_link.php');
	...
	vc_lean_map('pricing_table', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/pricing_table.php');
	...
	vc_lean_map('pricing_column', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/pricing_column.php');
	...
	vc_lean_map('carousel', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/carousel.php');
	...
	// Social Buttons
	vc_lean_map('social_buttons', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/social_buttons.php');
	...
	vc_add_param("vc_gallery",array(
	      ...
	      "value" => array(
	         ...
	         esc_html__("Republicpg Slider Style", "js_composer") => "republicpgslider_style",
	...
	vc_add_param("vc_gallery",array(
	 ...
	 vc_add_param("vc_gallery",array(
	      ...
	      "value" => Array(esc_html__("Yes, please", "js_composer") => 'true'),
	      "dependency" => Array('element' => "type", 'value' => array('republicpgslider_style'))
	  ));
		vc_add_param("vc_gallery",array(
	      "type" => 'checkbox',
	...
	vc_add_param("vc_gallery",array(
	      ...
	      "description" => esc_html__("Enabling this will cause your image grid to bypass the default Blueprint image cropping which varies based on the defined Masonry Sizing field. The result will be a traditional masonry layout rather than a structured grid", "js_composer"),
	      "value" => Array(esc_html__("Yes, please", "js_composer") => 'true'),
	...
	vc_add_param("vc_gallery",array(
	      ...
	      'save_always' => true,
	      "dependency" => Array('element' => "type", 'value' => array('republicpgslider_style', 'flexslider_style', 'flickity_style'))
	));
	vc_add_param("vc_gallery",array(
	...
	// Text With Icon
	vc_lean_map('text-with-icon', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/text-with-icon.php');


	// Fancy UL
	vc_lean_map('fancy-ul', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/fancy-ul.php');
	...
	vc_lean_map('morphing_outline', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/morphing_outline.php');


	// Republicpg Item Price
	class WPBakeryShortCode_Republicpg_Food_Menu_Item extends WPBakeryShortCode { }
	vc_lean_map('republicpg_food_menu_item', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_food_menu_item.php');



	// Republicpg Btn
	class WPBakeryShortCode_Republicpg_Btn extends WPBakeryShortCode { }

	vc_lean_map('republicpg_btn', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_btn.php');



	//Republicpg Icon
	class WPBakeryShortCode_Republicpg_Icon extends WPBakeryShortCode { }

	vc_lean_map('republicpg_icon', null, $republicpg_template_dir . '/republicpg/republicpg-vc-addons/republicpg_maps/republicpg_icon.php');
