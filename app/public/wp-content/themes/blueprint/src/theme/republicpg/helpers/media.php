...
if ( ! function_exists( 'republicpg_add_image_sizes' ) ) {

	function republicpg_add_image_sizes() {
		...
		add_image_size( 'republicpg_small_square', 140, 140, true );
...
add_action( 'after_setup_theme', 'republicpg_add_image_sizes' );
...
function republicpg_list_thumbnail_sizes() {
...
if ( ! function_exists( 'republicpg_auto_gallery_lightbox' ) ) {
	function republicpg_auto_gallery_lightbox( $content ) {
...
if ( ! empty( $republicpg_options['default-lightbox'] ) && $republicpg_options['default-lightbox'] == '1' ) {
	add_filter( 'the_content', 'republicpg_auto_gallery_lightbox' );

	add_filter( 'body_class', 'republicpg_auto_gallery_lightbox_class' );
	function republicpg_auto_gallery_lightbox_class( $classes ) {
		// add 'class-name' to the $classes array
		$classes[] = 'republicpg-auto-lightbox';
 ...
 // -----------------------------------------------------------------#
function republicpg_add_attachment_field_credit( $form_fields, $post ) {

	$form_fields['image-url'] = array(
		'label' => 'Image URL',
		'input' => 'text',
		'value' => get_post_meta( $post->ID, 'republicpg_image_gal_url', true ),
		'helps' => '',
	);
	 ...
	 $form_fields['shape-bg-color'] = array(
		 'label' => 'BG Color',
		 'input' => 'text',
		 'value' => esc_attr( get_post_meta( $post->ID, 'republicpg_particle_shape_bg_color', true ) ),
	...
	$meta                                     = get_post_meta( $post->ID, 'republicpg_image_gal_masonry_sizing', true );
	...
	$meta                  = get_post_meta( $post->ID, 'republicpg_particle_shape_color_mapping', true );
	...
	$meta            = get_post_meta( $post->ID, 'republicpg_particle_shape_density', true );
	...
	$meta          = get_post_meta( $post->ID, 'republicpg_particle_shape_color_alpha', true );
	...
	$form_fields['masonry-image-sizing'] = array(
		...
		'helps' => '',
		'value' => get_post_meta( $post->ID, 'republicpg_image_gal_masonry_sizing', true ),
	);
	...
	$form_fields['shape-color-mapping'] = array(
		...
		'helps' => '',
		'value' => get_post_meta( $post->ID, 'republicpg_particle_shape_color_mapping', true ),
	);
	...
	$form_fields['shape-color-alpha'] = array(
		...
		'helps' => '',
		'value' => get_post_meta( $post->ID, 'republicpg_particle_shape_color_alpha', true ),
	);

	$form_fields['shape-particle-color'] = array(
		'label' => 'Particle Color',
		'input' => 'text',
		'value' => esc_attr( get_post_meta( $post->ID, 'republicpg_particle_shape_color', true ) ),
	...
	$form_fields['shape-density'] = array(
		...
		'value' => get_post_meta( $post->ID, 'republicpg_particle_shape_density', true ),
	);

	$form_fields['shape-max-particle-size'] = array(
		'label' => 'Max Particle Size',
		'input' => 'text',
		'value' => get_post_meta( $post->ID, 'republicpg_particle_max_particle_size', true ),
		'helps' => 'The default is 3',
	);

	return $form_fields;
}
 add_filter( 'attachment_fields_to_edit', 'republicpg_add_attachment_field_credit', 10, 2 );

function republicpg_add_attachment_field_credit_save( $post, $attachment ) {
	if ( isset( $attachment['image-url'] ) ) {
		$image_url_sanitized = sanitize_text_field( $attachment['image-url'] );
		update_post_meta( $post['ID'], 'republicpg_image_gal_url', $image_url_sanitized );
	}
	...
	if ( isset( $attachment['masonry-image-sizing'] ) ) {
		...
		update_post_meta( $post['ID'], 'republicpg_image_gal_masonry_sizing', $masonry_image_sizing_sanitized );
	}

	if ( isset( $attachment['shape-bg-color'] ) ) {
		$shape_bg_color_sanitized = sanitize_text_field( $attachment['shape-bg-color'] );
		update_post_meta( $post['ID'], 'republicpg_particle_shape_bg_color', $shape_bg_color_sanitized );
	}
	...
	if ( isset( $attachment['shape-particle-color'] ) ) {
		...
		update_post_meta( $post['ID'], 'republicpg_particle_shape_color', $shape_particle_color_sanitized );
	}
	if ( isset( $attachment['shape-color-mapping'] ) ) {
		$shape_color_mapping_sanitized = sanitize_text_field( $attachment['shape-color-mapping'] );
		update_post_meta( $post['ID'], 'republicpg_particle_shape_color_mapping', $shape_color_mapping_sanitized );
	}
	if ( isset( $attachment['shape-color-alpha'] ) ) {
		$shape_color_alpha_sanitized = sanitize_text_field( $attachment['shape-color-alpha'] );
		update_post_meta( $post['ID'], 'republicpg_particle_shape_color_alpha', $shape_color_alpha_sanitized );
	}
	if ( isset( $attachment['shape-density'] ) ) {
		$shape_density_sanitized = sanitize_text_field( $attachment['shape-density'] );
		update_post_meta( $post['ID'], 'republicpg_particle_shape_density', $shape_density_sanitized );
	}
	if ( isset( $attachment['shape-max-particle-size'] ) ) {
		...
		update_post_meta( $post['ID'], 'republicpg_particle_max_particle_size', $shape_max_particle_size_sanitized );
	}
	return $post;
}
 add_filter( 'attachment_fields_to_save', 'republicpg_add_attachment_field_credit_save', 10, 2 );
...
if ( ! function_exists( 'republicpg_options_img' ) ) {

	function republicpg_options_img( $image_arr_or_str ) {

		// dummy data import from external
		if ( isset( $image_arr_or_str['thumbnail'] ) && strpos( $image_arr_or_str['thumbnail'], 'http://republicpg.com' ) !== false && $_SERVER['SERVER_NAME'] != 'republicpg.com' ) {
