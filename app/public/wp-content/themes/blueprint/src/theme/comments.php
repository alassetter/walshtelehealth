...
<?php if ( have_comments() ) : ?>
	<h3 id="comments"><?php  if(!empty($options['theme-skin']) && $options['theme-skin'] == 'ascend') echo '<span><i>'. esc_html__("Join the discussion", 'blueprint').'</i></span>' ?> <?php comments_number(esc_html__('No Comments','blueprint'), esc_html__('One Comment', 'blueprint'), esc_html__('% Comments', 'blueprint') );?></h3>
...
$comment_label = ($form_style == 'minimal') ? '<label for="comment">' . esc_html__('My comment is..', 'blueprint') . '</label>' : null;
$consent  = empty( $commenter['comment_author_email'] ) ? '' : ' checked="checked"';
$args = array(
  'id_form'           => 'commentform',
  'id_submit'         => 'submit',
  'title_reply'       => __( 'Leave a Reply', 'blueprint' ),
  'title_reply_to'    => __( 'Leave a Reply to %s', 'blueprint' ),
  'cancel_reply_link' => __( 'Cancel Reply', 'blueprint' ),
  'label_submit'      => __( 'Submit Comment', 'blueprint' ),
  ...
  'must_log_in' => '<p class="must-log-in">' .
    sprintf(
      __( 'You must be <a href="%s">logged in</a> to post a comment.', 'blueprint' ),
      wp_login_url( apply_filters( 'the_permalink', esc_url(get_permalink()) ) )
    ) . '</p>',

  'logged_in_as' => '<p class="logged-in-as">' .
    sprintf(
    __( 'Logged in as <a href="%1$s">%2$s</a>. <a href="%3$s" title="Log out of this account">Log out?</a>', 'blueprint' ),
  ...
  'fields' => apply_filters( 'comment_form_default_fields', array(
    ...
    'author' =>
      ...
      '<label for="author">' . __( 'Name', 'blueprint' ) .
      ' <span class="required">*</span></label> ' .
      '<input id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) .
      '" size="30" /></div>',

    'email' =>
      '<div class="col span_4"><label for="email">' . __( 'Email', 'blueprint' ) .
      ' <span class="required">*</span></label>' .
