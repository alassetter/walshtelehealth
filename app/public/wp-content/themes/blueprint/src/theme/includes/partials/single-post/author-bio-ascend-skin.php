...
<div id="author-bio" data-midnight="dark" class="<?php echo esc_attr( $fw_class ); // WPCS: XSS ok. ?> <?php if ( empty( $next_post ) || $next_post_button == 'off' || $fullscreen_header == false && $next_post_button == 'off' ) { echo 'no-pagination';} ?> ">
  <div class="span_12">
	...
	?>
	<div id="author-info">
	  <h3><span><?php if ( $theme_skin == 'ascend' ) { echo '<i>' . esc_html__( 'Author', 'blueprint' ) . '</i>'; } else { _e( 'About', 'blueprint' ); } ?></span> <?php the_author(); ?></h3>
	  <p><?php the_author_meta( 'description' ); ?></p>
	</div>
	<?php
	if ( $theme_skin == 'ascend' ) {
		echo '<a href="' . get_author_posts_url( get_the_author_meta( 'ID' ) ) . '" data-hover-text-color-override="#fff" data-hover-color-override="false" data-color-override="#000000" class="republicpg-button see-through-2 large">' . esc_html__( 'More posts by', 'blueprint' ) . ' ' . get_the_author() . ' </a>'; }
	?>
	<div class="clear"></div>
  </div>
</div>
