...
$remove_single_post_republicpg_love    = ( ! empty( $options['blog_remove_single_republicpg_love'] ) ) ? $options['blog_remove_single_republicpg_love'] : '0';
...
if ( empty( $bg ) && empty( $bg_color ) && $single_post_header_inherit_fi != '1' ) { ?>
  ...
  <div class="not-loaded default-blog-title fullscreen-header hentry" id="page-header-bg" data-midnight="light" data-alignment-v="middle" data-alignment="center" data-parallax="0" data-height="450" data-remove-post-date="<?php echo esc_attr( $remove_single_post_date ); ?>" data-remove-post-author="<?php echo esc_attr( $remove_single_post_author ); ?>" data-remove-post-comment-number="<?php echo esc_attr( $remove_single_post_comment_number ); ?>">
			...
			if ( ( $post->post_type == 'post' && is_single() ) && $theme_skin == 'material' ) {
				...
				if ( ! empty( $categories ) ) {
					...
					foreach ( $categories as $category ) {
						$output .= '<a class="' . esc_attr( $category->slug ) . '" href="' . esc_url( get_category_link( $category->term_id ) ) . '" alt="' . esc_attr( sprintf( __( 'View all posts in %s', 'blueprint' ), $category->name ) ) . '">' . esc_html( $category->name ) . '</a>';
					}
					echo trim( $output );
				}
			}
	...
	if ( $button_styling == 'default' ) {
		echo '<div class="scroll-down-wrap"><a href="#" class="section-down-arrow"><i class="icon-blueprint-down-arrow icon-default-style"> </i></a></div>';
	} elseif ( $button_styling == 'slightly_rounded' || $button_styling == 'slightly_rounded_shadow' ) {
		echo '<div class="scroll-down-wrap no-border"><a href="#" class="section-down-arrow"><svg class="republicpg-scroll-icon" viewBox="0 0 30 45" enable-background="new 0 0 30 45">
	...
	<div id="single-below-header" class="<?php echo esc_attr( $fullscreen_class ); ?> custom-skip" data-remove-post-comment-number="<?php echo esc_attr( $remove_single_post_comment_number ); ?>">
	  <?php if ( $blog_social_style != 'fixed_bottom_right' ) { ?>
		<span class="meta-share-count"><i class="icon-default-style steadysets-icon-share"></i>
			<?php
			echo '<a href=""><span class="share-count-total">0</span> <span class="plural">' . esc_html__( 'Shares', 'blueprint' ) . '</span> <span class="singular">' . esc_html__( 'Share', 'blueprint' ) . '</span> </a>';
			republicpg_blog_social_sharing();
			?>
		 </span>
		<?php } else { ?>
		  <?php if ( $remove_single_post_republicpg_love != '1' ) { ?>
			  <span class="meta-love"><span class="n-shortcode"> <?php echo republicpg_love( 'return' ); ?>  </span></span>
			<?php } ?>
		<?php } ?>
	  <span class="meta-category"><i class="icon-default-style steadysets-icon-book2"></i> <?php the_category( ', ' ); ?></span>
	  <span class="meta-comment-count"><i class="icon-default-style steadysets-icon-chat-3"></i> <a href="<?php comments_link(); ?>"><?php comments_number( esc_html__( 'No Comments', 'blueprint' ), esc_html__( 'One Comment ', 'blueprint' ), esc_html__( '% Comments', 'blueprint' ) ); ?></a></span>
