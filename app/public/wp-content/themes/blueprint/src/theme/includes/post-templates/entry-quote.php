...
<article id="post-<?php the_ID(); ?>" <?php post_class( $republicpg_post_class_additions . ' quote' ); ?> >
      ...
      /**
  		 ...
  		 */
			...
			if ( ! is_single() ) { ?>
				...
				<?php
				$quote        = get_post_meta( $post->ID, '_republicpg_quote', true );
				$quote_author = get_post_meta( $post->ID, '_republicpg_quote_author', true );
			...
			<div class="content-inner">
				...
				if ( is_single() && ! empty( $below_content ) ) {
					...
					<div class="quote-below-content">	
						<?php the_content( '<span class="continue-reading">' . __( 'Read More', 'blueprint' ) . '</span>' ); ?>
					</div>
				<?php } 
