...
class Recent_Projects_Widget extends WP_Widget {
	...
	function __construct() {
		$widget_ops = array('classname' => 'recent_projects_widget', 'description' => __( "The most recent projects on your site.",'blueprint'));
		parent::__construct('recent-projects', esc_html__('Recent Projects','blueprint'), $widget_ops);
	...
	function widget($args, $instance) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? esc_html__('Recent Projects','blueprint') : $instance['title']);
...
<?php
	...
	function form( $instance ) {
		...
		if ( !isset($instance['number']) || !$number = (int) $instance['number'] )
			...
			$number = 9;
?>
		<p><label for="<?php echo wp_kses_post( $this->get_field_id('title') ); ?>"><?php _e('Title:', 'blueprint'); ?></label>
		...
		<p><label for="<?php echo esc_attr( $this->get_field_id('number') ); ?>"><?php _e('Number of projects to show:', 'blueprint'); ?></label>
		...
		<small><?php _e('Change in increments of 3 (at most 9)', 'blueprint'); ?></small></p>
<?php
	}
}
