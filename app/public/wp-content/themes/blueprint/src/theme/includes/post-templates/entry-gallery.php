...
if ( isset( $GLOBALS['republicpg_blog_std_style'] ) && $GLOBALS['republicpg_blog_std_style'] != 'inherit' ) {
	$blog_standard_type = $GLOBALS['republicpg_blog_std_style'];
} else {
...
if ( isset( $GLOBALS['republicpg_blog_masonry_style'] ) && $GLOBALS['republicpg_blog_masonry_style'] != 'inherit' ) {
	$masonry_type = $GLOBALS['republicpg_blog_masonry_style'];
} else {
...
if ( $using_masonry == true && ! is_single() ) {
	$republicpg_post_class_additions = $masonry_item_sizing . ' masonry-blog-item';
} else {
	$republicpg_post_class_additions = $masonry_item_sizing;
}

?>

<article id="post-<?php the_ID(); ?>" <?php post_class( $republicpg_post_class_additions ); ?>>
	...
	<div class="inner-wrap animated">
		...
		<div class="post-content">
			...
			if ( ! is_single() ) {
				...
				<div class="post-meta <?php echo esc_attr( $extra_class ); ?>">
					...
					<?php if ( ( $masonry_type == 'classic_enhanced' && $using_masonry == true ) ) { ?> 
						<span class="meta-author"> <a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>"> <i class="icon-default-style icon-blueprint-m-user"></i> <?php the_author(); ?></a> </span> <span class="meta-comment-count">  <a href="<?php comments_link(); ?>">
			...
			$enable_gallery_slider = get_post_meta( get_the_ID(), '_republicpg_gallery_slider', true );
			 ...
			 if ( ! is_single() ) { ?>
					...
					<div class="post-author">
						...
						if ( ! empty( $categories ) ) {
							...
							echo '<span class="in">' . esc_html__( 'In', 'blueprint' ) . ' </span>';
				...
				if ( ! is_single() ) {
					...
					if ( empty( $post->post_excerpt ) && $use_excerpt != 'true' ) {
									the_content( '<span class="continue-reading">' . __( 'Continue Reading', 'blueprint' ) . '</span>' );
					}
						...
						echo '</div>';
						echo '<a class="more-link" href="' . esc_url( get_permalink() ) . '"><span class="continue-reading">' . __( 'Continue Reading', 'blueprint' ) . '</span></a>';
					}
					...
					?>
						...
						if ( is_single() ) {
							...
							the_content( '<span class="continue-reading">' . __( 'Read More', 'blueprint' ) . '</span>' );
						}
						?>
						...
						if ( $options['display_tags'] == true ) {
							...
							if ( is_single() && has_tag() ) {
								...
								echo '<div class="post-tags"><h4>' . esc_html__( 'Tags:', 'blueprint' ) . '</h4>';
								the_tags( '', '', '' );
				...
				if ( $meta_overlaid_style == true ) {
					...
					if ( ! empty( $enable_gallery_slider ) && $enable_gallery_slider == 'on' && ! ( is_single() && $hide_featrued_image == '1' ) ) {
						...
						if ( ! ( $using_masonry == true && $masonry_type == 'classic_enhanced' ) && ! ( $using_masonry == true && $masonry_type == 'material' ) ) {
							...
							echo '<div class="republicpg-flickity ' . $flickity_masonry_class . ' not-initialized" data-controls><div class="flickity-viewport"> <div class="flickity-slider">';
							foreach ( $gallery_ids as $image_id ) {
					...
					<div class="article-content-wrap">
						...
						<div class="post-header">
							...
							<?php if ( ! ( $masonry_type == 'classic_enhanced' && $using_masonry == true ) && ! ( $masonry_type == 'material' && $using_masonry == true ) ) { ?> 
								<span class="meta-author"><span><?php echo esc_html__( 'By', 'blueprint' ); ?></span> <?php the_author_posts_link(); ?></span> <span class="meta-category">| <?php the_category( ', ' ); ?></span> <span class="meta-comment-count">| <a href="<?php comments_link(); ?>">
						...
						// excerpt
							...
							if ( $using_masonry == true && $masonry_type == 'classic_enhanced' ) {
								...
								if ( $masonry_item_sizing == 'wide_tall' && ! empty( $post->post_excerpt ) ) {
									...
									echo republicpg_excerpt( $excerpt_length * 2 );
								} else {
									echo republicpg_excerpt( $excerpt_length );
								}
							...
							if ( ! ( $using_masonry == true && $masonry_type == 'material' ) ) {
								echo '<a class="more-link" href="' . esc_url( get_permalink() ) . '"><span class="continue-reading">' . __( 'Read More', 'blueprint' ) . '</span></a>';
							}
						}
					}
					?>
