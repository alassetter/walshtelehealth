...
window.vc.filters = {
    ...
    function($) {
        void 0 === window.vc && (window.vc = {}), vc.ShortcodesBuilder = function(models) {
            ...
            renderShortcode: function($html, model) {
                ...
                /*republicpg addition - do not trigger until after page is loaded */
        ...
        vc.EditElementPanelView = vc.PanelView.vcExtendUI(vc.HelperAjax).vcExtendUI(vc.ExtendPresets).vcExtendUI(vc.ExtendTemplates).vcExtendUI(vc.HelperPrompts).extend({
            ...
            buildParamsContent: function(data) {
               ...
               $('.wpb-textinput.background_color, .gallery_widget_attached_images_ids').change(function(){
                ...
                var $using_republicpg_fspr = ( vc.frame_window.jQuery('body.republicpg_using_pfsr').length > 0 ) ? true : false;
                ...
                function verticallyCenterColumns() {
                     ...
                     if($('select[name="type"]').val() == 'full_width_content' && $using_republicpg_fspr) {

                        $('select[name="full_screen_row_position"] .full_height').show();

                     } else if($('select[name="type"]').val() == 'full_width_background' && $using_republicpg_fspr) {
                    ...
                    if($using_republicpg_fspr && $('select#_republicpg_full_screen_rows_row_bg_animation').val() == 'ken_burns')
              ...
              //cta_button
                if(this.$tabs.parent().find('.wpb_tabs_nav li.cta-button').length > 0) {
                  ...
                  if(cta_button_text.length > 0) {
                    this.$tabs.parent().find('.wpb_tabs_nav li.cta-button').html('<a class="republicpg-button medium regular-button '+ cta_button_tab_color +'" data-color-override="false" target="_blank" href="'+ cta_button_link +'">'+ cta_button_text +'</a>');
                  } else {
    ...
    /****Icon list*****/
    (window.jQuery), function($) {
        window.InlineShortcodeView_republicpg_icon_list = window.InlineShortcodeView_vc_accordion.extend({
            updateNumericalIcons: function() {
              var $that = this;
              setTimeout(function(){
                $that.$el.find('> .republicpg-icon-list .list-icon-holder[data-icon_type="numerical"]').each(function(i){
            ...
            render: function() {
                return _.bindAll(this, "stopSorting"), 
                this.$accordion = this.$el.find("> .republicpg-icon-list"), 
                window.InlineShortcodeView_vc_accordion.__super__.render.call(this), this
            },
            addElement: function(e) {
                e && e.preventDefault(), (new vc.ShortcodesBuilder).create({
                    shortcode: "republicpg_icon_list_item",
                    params: {
                        title: window.i18nLocale.section
                    },
                    parent_id: this.model.get("id")
                }).render();
                this.$el.find("> .republicpg-icon-list").removeClass('completed');
    ...
    (window.jQuery), function($) {
        window.InlineShortcodeView_republicpg_icon_list_item = window.InlineShortcodeView_vc_accordion_tab.extend({
        ...
        window.InlineShortcodeView_carousel = window.InlineShortcodeView_vc_accordion.extend({
          ...
          updated: function() {
              ...
              if(this.$el.find('.flickity-slider').length > 0) {
                ...
                if( this.$el.find('> .republicpg-carousel').is('[data-column-color]') ) {
                  
                  var $carouselColColor = this.$el.find('> .republicpg-carousel').attr('data-column-color');
