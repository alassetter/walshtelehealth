...
HTML;
	...
	public function getAllTemplates() {
		...
		if ( ! empty( $default_templates ) ) {
			$arr_category = array(
				...
				'category_name' => __( 'Blueprint Studio', 'js_composer' ),
				'category_description' => __( 'Append predefined Blueprint templates to the current layout', 'js_composer' ),
				'category_weight' => 11,
				/*republicpg addition end*/
			);
			$category_templates = array();
	...
	public function renderTemplateListItem( $template ) {
		...
		/*republicpg addition - adding in preview img*/
		$preview_img = esc_attr( isset( $template['image'] ) &&  $template['image'] != '' ? $template['image'] : get_template_directory_uri() .'/republicpg/republicpg-vc-addons/img/templates/no-img.jpg'  );
