				...
				<div class="col <?php echo esc_attr( $span_num ) . ' ' .  esc_attr( $masonry_item_sizing ); ?> element" data-project-cat="<?php echo esc_attr( $project_cats ); ?>" <?php if ( ! empty( $project_accent_color ) ) { echo 'data-project-color="' .  esc_attr( $project_accent_color ) . '"'; } else { echo 'data-default-color="true"';} ?> data-title-color="<?php echo esc_attr( $project_title_color ); ?>" data-subtitle-color="<?php echo  esc_attr( $project_subtitle_color ); ?>">
					...
					<div class="inner-wrap animated" data-animation="<?php echo  esc_attr( $load_in_animation ); ?>">
								...
								<div class="work-info"> 
									...
									<div class="vert-center">
												...
												if ( ! empty( $video_embed ) || ! empty( $video_m4v ) ) {
													...
													echo republicpg_portfolio_video_popup_link( $post, $project_style, $project_style, $video_embed, $video_m4v );

												}

												// image
												else {
													echo '<a href="' . esc_url( $featured_image[0] ) . '" class="pretty_photo default-link" >' . esc_html__( 'View Larger', 'blueprint' ) . '</a> ';
												}
							...
							<div class="work-item style-2" data-custom-content="<?php echo esc_url( $using_custom_content ); ?>">
												...
												if ( ! empty( $custom_thumbnail ) ) {
													echo '<img class="custom-thumbnail" src="' . republicpg_ssl_check( esc_url( $custom_thumbnail ) ) . '" alt="' . get_the_title() . '" />';
												} else {
	...
	if ( ! empty( $video_embed ) || ! empty( $video_m4v ) ) {
		...
		if ( ! empty( $options['portfolio_extra_pagination'] ) && $options['portfolio_extra_pagination'] == '1' ) {
			...
			if ( $total_pages > 1 ) {
				...
				echo '<div id="pagination" class="' . $fw_pagination . ' ' . $masonry_padding . esc_attr( $infinite_scroll_class ) . '" data-is-text="' . esc_attr__( 'All items loaded', 'blueprint' ) . '">'; // WPCS: XSS ok.
			...
			if ( get_next_posts_link() || get_previous_posts_link() ) {
				...
				echo '<div id="pagination" class="' . $fw_pagination . ' ' . $masonry_padding . esc_attr( $infinite_scroll_class ) . '" data-is-text="' . esc_attr__( 'All items loaded', 'blueprint' ) . '">
