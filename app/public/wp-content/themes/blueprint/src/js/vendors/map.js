...
jQuery(document).ready(function($){
	...
	window.mapAPI_Loaded = function() {
		...
		for(var i = 0; i < $('.republicpg-google-map').length; i++) {
			 infoWindows[i] = [];
		}

		$('.republicpg-google-map').each(function(i){
      ...
      var $gMapsAnimatedSelector = $('.col.has-animation[data-animation="grow-in"] .republicpg-google-map');
      var gMapsInterval = [];
	...
	if(typeof google === 'object' && typeof google.maps === 'object') {
 		...
 		if(republicpgLove.mapApiKey.length > 0) {
 			$.getScript('https://maps.googleapis.com/maps/api/js?sensor=false&key='+republicpgLove.mapApiKey+'&callback=mapAPI_Loaded');
 		} else {
		  ...
		  $('.map-marker-list.'+map_id).each(function(){
		        ...
		        var enableAnimation = $('#'+map_id).attr('data-enable-animation');
		      	...
		      	$(this).find('.map-marker').each(function(i){
					...
					//republicpg marker 
					if($('#'+map_id).is('[data-marker-style="republicpg"]')) {
