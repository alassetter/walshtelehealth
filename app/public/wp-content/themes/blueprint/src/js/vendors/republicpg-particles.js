...
var $shapes = [];
var $selector = ($('.republicpg-box-roll').length > 0) ? '.republicpg-box-roll .shape': '.republicpg-particles .shape';
...
var Nodes = {
  ...
  disableExplosion: $('#page-header-bg .republicpg-particles').attr('data-disable-explosion'),
  rotateTimer: parseInt($('#page-header-bg .republicpg-particles').attr('data-rotation-timing')),
  ...
  init: function(canvasID) {
    ...
    this.canvas.width = ($(canvasID).parents('.republicpg-box-roll').length > 0 ) ? window.innerWidth : $(canvasID).parents('#page-header-bg').outerWidth(true);
    this.canvas.height = ($(canvasID).parents('.republicpg-box-roll').length > 0 ) ? window.innerHeight : $(canvasID).parents('#page-header-bg').outerHeight(true);
    ...
    */
    ...
    //default autorotate
    if($('#page-header-bg .republicpg-particles').attr('data-rotation-timing').length < 1) Nodes.rotateTimer = 5500;

    if($(canvasID).parents('.republicpg-box-roll').length > 0) {
	    ...
	    //});
    	 ...
    	 $(canvasID).parents('.republicpg-particles').on('mousemove',function(e){
	    	Nodes.mouse.x = e.clientX - $(this).offset().left;
	    	Nodes.mouse.y = e.clientY - $(this).offset().top + $(window).scrollTop();
	    });
	    $(canvasID).parents('.republicpg-particles').on('mouseout',function(){
	    	Nodes.mouse.x = 1000;
	  		Nodes.mouse.y = -1000;
	  		Nodes.mouse.down = false;
	    });
    ...
    if($('#page-header-bg:not(.fullscreen-header)').length > 0 && $(window).width() < 1000) {
       $(window).load(function(){
        setTimeout(function(){
          ...
          Nodes.canvas.width = ($(canvasID).parents('.republicpg-box-roll').length > 0 ) ? window.innerWidth : $(canvasID).parents('#page-header-bg').outerWidth(true);
          Nodes.canvas.height = ($(canvasID).parents('.republicpg-box-roll').length > 0 ) ? window.innerHeight : $(canvasID).parents('#page-header-bg').outerHeight(true);
  ...
  draw: function() {
	  	...
	  	//if($('.no-scroll').length > 0) {
	  		...
	  		//$(Nodes.canvasID).parents('.republicpg-box-roll').find('.canvas-bg.topBoxOut').length == 0 && $(Nodes.canvasID).parents('.republicpg-box-roll').find('.canvas-bg.topBoxIn').length == 0 
  ...
  particleRotateLogic: function(seek) {
         ...
         if(this.reactionSensitivity > 4) this.reactionSensitivity = (window.innerWidth > 690) ? 4 : 1; 
			   ...
			   //update pagination
			   var $selector = ($('.republicpg-box-roll').length > 0) ? '.republicpg-box-roll': '.republicpg-particles';
  ...
  canvasBgColor: function(){
  		...
  		jQuery(Nodes.canvasID).parents('.republicpg-particles').find('.canvas-bg').css({
...
function boxRoll(e,d) {
	...
	if(perspect == 'not-rolled' && animating == 'false' && d == -1 && $('.republicpg-box-roll canvas[data-loaded="true"]').length > 0) {
		perspect = 'rolled';
		animating = 'true';
		//stop mouse movement
		Nodes.mouse.x = 2000;
